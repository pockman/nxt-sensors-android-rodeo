...
import blueclean.bolas.eus.NXTDriver;
...
public class NXTCommand implements NXTProtocol {
  ...
  /**
   * Instantiates a new nXT command.
   */
  private NXTCommand() {
  }
   ...
   *
   * @param bufferNumber 0 = poll buffer (low-speed) 1 = high-speed buffer
   * @return the byte
   */
   ...
   *
   * @param bufferNumber 0 = poll buffer (low-speed) 1 = high-speed buffer
   * @param commandLength Number of bytes obtained from pollLength()
   * @return the byte[]
   */
   ...
   * @param mode - Setting the modes MOTORON, BRAKE, and/or REGULATED. This parameter is a bitfield, so
  ...
  public static void open() throws RuntimeException {
    singleton.nxtComm = NXTDriver.getInstance();
    try {
