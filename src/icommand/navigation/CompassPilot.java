...
/**
 * The iCommand CompassPilot is simpler than the NXJ CompassPilot. This version rotates using the
 * compass but does not move forward/backward with compass assistance.
 * 
 * @author BB
 * 
 */
public class CompassPilot extends Pilot {
   ...
   */
  public CompassPilot(SensorPort compassPort, double wheelDiameter, double trackWidth,
   ...
   * @param rightMotor the right motor
   */
  public CompassPilot(CompassSensor compass, double wheelDiameter, double trackWidth,
      Motor leftMotor, Motor rightMotor) {
  ...
  /* (non-Javadoc)
   * @see icommand.navigation.Pilot#getAngle()
   */
  public int getAngle() {
  ...
  /**
   * The rotate method takes a current reading from the compass then rotates the desired number of
   * degrees. NOTE: immediateReturn is not functional currently (always waits until done rotating).
   *
  ...
  public void rotate(int angle, boolean immediateReturn) {
    ...
    /*
     * if(immediateReturn) return; while(Math.abs(getError(_heading)) > THRESH) { Thread.yield();
