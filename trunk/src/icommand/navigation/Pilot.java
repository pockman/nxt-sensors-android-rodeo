...
/**
 * The Pilot class is a software abstraction of the Pilot mechanism of a NXT robot. It contains
 * methods to control robot movents: travel forward or backward in a straight line or a circular
 * path or rotate to a new direction. <br>
 * Note: this class will only work with two independently controlled Pilot motors to steer
 * differentially, so it can rotate within its own footprint (i.e. turn on one spot).<br>
 * It can be used with robots that have reversed motor design: the robot moves in the direction
 * opposite to the the dirction of motor rotation. Uses the Motor class, which regulates motor speed
 * using the NXT motor's built in tachometer. <br>
 * Some methods optionally return immediately so the thread that called the method can monitor
 * sensors and call stop() if necessary. <br>
...
public class Pilot {
   ...
   **/
  private final double _turnRatio;
   ...
   *
   * @param wheelDiameter Diameter of the tire, in any convenient units. (The diameter in mm is usually printed
   * on the tire).
   * @param trackWidth Distance between center of right tire and center of left tire, in same units as
   * wheelDiameter
   * @param leftMotor the left motor
   * @param rightMotor the right motor
   */
  public Pilot(double wheelDiameter, double trackWidth, Motor leftMotor, Motor rightMotor) {
    _left = leftMotor;
    _right = rightMotor;
   ...
   * @param wheelDiameter Diameter of the tire, in any convenient units. (The diameter in mm is usually printed
   * on the tire).
   * @param trackWidth Distance between center of right tire and center of left tire, in the same units as
   * wheelDiameter
   * @param leftMotor the left motor
   * @param rightMotor the right motor
  ...
  /**
   * Rotates the NXT robot through a specific angle; Rotates left if angle is positive, right if
   * negative, Returns when angle is reached. Wheels turn in opposite directions producing a zero
   * radius turn.
   * 
   * @param angle
   *          degrees. Positive angle rotates to the left (clockwise); negative to the right. <br>
  ...
  /**
   * Rotates the NXT robot through a specific angle; Rotates left if angle is positive, right if
   * negative; Returns immediately iff immediateReturn is true. Wheels turn in opposite directions
   * producing a zero radius turn.
   * 
   * @param angle
   *          degrees. Positive angle rotates to the left; negative to the right. <br>
   *          Requires correct values for wheel diameter and track width.
   ...
   */
  public int getAngle() {
    return _parity * (int) Math.round((getRightCount() - getLeftCount()) / (2 * _turnRatio));
  }
   ...
   * 
   * @param distance
   *          of robot movement. Unit of measure for distance must be same as wheelDiameter and
   *          trackWidth
   **/
  ...
  /**
   * Moves the NXT robot a specific distance; if immediateReturn is true, method returns
   * immediately. <br>
   * A positive distance causes forward motion; negative distance moves backward.
   *
   * @param distance the distance
   * @param immediateReturn If true, method returns immediately, and robot stops after traveling the distance. If
   * false, method returns immediately.
   */
   ...
   * The center of the turning circle is on the right side of the robot iff parameter turnRate is
   * negative; <br>
   * turnRate values are between -200 and +200;
   * 
   * @param turnRate
   *          If positive, the left wheel is on the inside of the turn. If negative, the left wheel
   *          is on the outside. This parameter determines the ratio of inner wheel speed to outer
   *          wheel speed (as a percent). <br>
   *          <I>Formula:</i> ratio = 100 - abs(turnRate). When the ratio is negative, the outer and
   *          inner wheels rotate in opposite directions.<br>
   *          Examples:
   *          <UL>
   *          <LI>steer(25)-> inner wheel turns at 75% of the speed of the outer wheel <br>
   *          <li>steer(100) -> inner wheel stops.<br>
   *          <li>steer(200) -> means that the inner wheel turns at the same speed as the outer
   *          wheel - a zero radius turn. <br>
   *          </UL>
   */
   ...
   * Range of turnRate values : -200 : 200 ; Robot will stop when total rotation equals angle. If
   * angle is negative, robot will move travel backwards.
   * 
   * @param turnRate
   *          If positive, the left wheel is on the inside of the turn. If negative, the left wheel
   *          is on the outside. This parameter determines the ratio of inner wheel speed to outer
   *          wheel speed (as a percent). <br>
   * @param angle
   *          the angle through which the robot will rotate and then stop. If negative, robot traces
   *          the turning circle backwards.
   */
  ...
  /**
   * Moves the NXT robot in a circular path, and stops when the direction it is facing has changed
   * by a specific angle; <br>
   * Returns immediately if immediateReturn is true. The robot will stop automatically when the turm
   * is complete. The center of the turning circle is on right side of the robot iff parameter
   * turnRate is negative <br>
   * turnRate values are between -200 and +200;
   * 
   * @param turnRate
   *          If positive, the left wheel is on the inside of the turn. If negative, the left wheel
   ...
   *          the turning circle backwards.
   * @param immediateReturn
   *          iff true, method returns immedediately.
   * 
   */
  public void steer(int turnRate, int angle, boolean immediateReturn) {
    ...
    double steerRatio = 1 - rate / 100.0f;
    ...
    double rotAngle = angle * _trackWidth * 2 / (_wheelDiameter * (1 - steerRatio));
