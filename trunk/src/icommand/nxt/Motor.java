...
import icommand.nxt.comm.OutputState;
...
public class Motor implements NXTProtocol {
  ...
  public int forward() {
    ...
    return nxtCommand.setOutputState(id, (byte) power, this.mode + MOTORON, regulationMode,
        turnRatio, runState, 0);
  }
  ...
  /**
   * Sets motor speed , in degrees per second; Up to 900 is posssible with 8 volts. NOTE: If using
   * LEGO firmware this will convert the number into power. 900 = 100% power, 450 = 50% power.
   * 
   * @param speed
  ...
  /**
   * Returns the rotation count for the motor. NOTE: If you are using leJOS NXJ firmware this will
   * return the same value as getRotationCount() because the leJOS NXJ firmware only uses one
   * Tachometer variable.
   * 
  ...
  /**
   * Returns the rotation count for the motor. The rotation count is something like the trip
   * odometer on your car. This count is reset each time a new function is called in Pilot.
   *
  ...
  /**
   * Block Tachometer Count is the count used to synchronize motors with one another. NOTE: If you
   * are using leJOS NXJ firmware this will always return 0 because this variable is not used in in
   * leJOS NXJ firmware. Use getRotationCount() instead.
   *
  ...
  /**
   * Causes the motor to rotate a certain count. The motor will also backtrack to the desired count
   * when done. NOTE: This method returns almost immediately if returnNow = true.
   * 
   * @param count
  ...
  public int rotate(long count, boolean returnNow) {
    ...
    // if(power > 0)
    if (count > 0)
      nxtCommand.setOutputState(id, power, this.mode + MOTORON, regulationMode, turnRatio,
          runState, (int) count); // Note using tachoLimit with Lego FW
    else
      nxtCommand.setOutputState(id, (byte) -power, this.mode + MOTORON, regulationMode, turnRatio,
          runState, (int) Math.abs(count)); // Note using tachoLimit with Lego FW
    if (returnNow)
      return status;
    else {
  ...
  /**
   * CURRENTLY NOT IMPLEMENTED! Use isMoving() for now. returns true when motor is rotating toward a
   * specified angle
   *
  ...
  /**
   * Causes the motor to rotate a certain count. This method returns after the rotation is
   * completed. NOTE: This method currently doesn't work well with the LEGO firmware.
   * 
   * @param count
   *          Number of counts to rotate motor.
   * @return Error value. 0 means succcess. See icommand.nxtcomm.ErrorMessages for details.
  ...
  /**
   * This method determines if and how the motor will be regulated. REGULATION_MODE_IDLE turns off
   * regulation REGULATION_MODE_MOTOR_SPEED regulates the speed (I think) REGULATION_MODE_MOTOR_SYNC
   * synchronizes this and any other motor with SYNC enabled.
   * 
   * @param mode
  ...
  /**
   * Rotates to a desired tacho count. Does not return until rotation done. Note: The tachocount can
  ...
  /**
   * Rotates to a desired tacho count. Returns before the rotation is done if you include true as
   * the argument.
   *
   * @param target the target
   * @param returnNow the return now
   * @return the int
   */
  public int rotateTo(long target, boolean returnNow) {
    // !! Probably innacuracy can creep into this if
  ...
  /**
   * Resets the block tachometer. NOTE: If you are using leJOS NXJ firmware this will not do
   * anything because BlockTacho is not used in the leJOS NXJ firmware. Use resetRotationCounter()
   * instead.
   *
   * @return Error value. 0 means succcess. See icommand.nxtcomm.ErrorMessages for details.
   * @deprecated
  ...
  public int stop() {
    this.runState = MOTOR_RUN_STATE_RUNNING;
    // this.regulationMode = REGULATION_MODE_MOTOR_SPEED;
    return nxtCommand.setOutputState(id, (byte) 0, BRAKE + MOTORON + REGULATED, regulationMode,
        turnRatio, runState, 0);
  }
